...
export const IntegratedAnalytics = ({ tasks, selectedDate }: IntegratedAnalyticsProps) => {
  ...
  const analyticsData = useMemo(() => {
    ...
    const selectedDateTasks = tasks.filter(task => 
      new Date(task.scheduled_date).toDateString() === selectedDateString
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className={`${themeColors.card} border-l-4 border-l-blue-500 transform hover:scale-105 transition-all duration-300`}>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className={`text-sm font-medium ${theme === 'midnight' || theme === 'obsidian' ? 'text-gray-300' : 'text-gray-600'}`}>
                  Today's Progress
                </p>
                <div className="flex items-center space-x-2">
                  ...
                  <Badge variant="secondary" className="text-xs">
                    {analyticsData.completedToday}/{analyticsData.totalToday}
                  </Badge>
                </div>
              </div>
        ...
        <Card className={`${themeColors.card} border-l-4 border-l-green-500 transform hover:scale-105 transition-all duration-300`}>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className={`text-sm font-medium ${theme === 'midnight' || theme === 'obsidian' ? 'text-gray-300' : 'text-gray-600'}`}>
            ...
            <p className={`text-xs mt-2 ${theme === 'midnight' || theme === 'obsidian' ? 'text-gray-400' : 'text-gray-500'}`}>
              Today's practice
            </p>
          </CardContent>
        </Card>

        <Card className={`${themeColors.card} border-l-4 border-l-purple-500 transform hover:scale-105 transition-all duration-300`}>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className={`text-sm font-medium ${theme === 'midnight' || theme === 'obsidian' ? 'text-gray-300' : 'text-gray-600'}`}>
                ...
                <span className={`text-2xl font-bold ${themeColors.text}`}>
                  {Math.floor(analyticsData.totalTime / 60)}h {analyticsData.totalTime % 60}m
                </span>
              </div>
        ...
        <Card className={`${themeColors.card} border-l-4 border-l-orange-500 transform hover:scale-105 transition-all duration-300`}>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className={`text-sm font-medium ${theme === 'midnight' || theme === 'obsidian' ? 'text-gray-300' : 'text-gray-600'}`}>
