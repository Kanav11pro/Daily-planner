...
export const QuoteSection = () => {
  ...
  return <div className={`relative bg-gradient-to-br ${themeColors.primary} rounded-3xl p-8 text-white overflow-hidden shadow-2xl border border-white/20`}>
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
        {powerfulQuotes.slice(0, 5).map((_, index) => <div key={index} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentQuote % 5 ? 'bg-yellow-300 scale-125' : 'bg-white/30'}`} />)}
      </div>
    </div>;
};