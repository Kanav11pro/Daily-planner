...
export const Header = ({ onAddTask }: HeaderProps) => {
  ...
  return (
    <header className={`${themeColors.card}/80 backdrop-blur-sm ${themeColors.border} border-b sticky top-0 z-40`}>
      <div className="container mx-auto px-4 py-4 sm:py-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
            <div className={`hidden md:flex items-center space-x-2 ${themeColors.accent} px-3 sm:px-4 py-2 rounded-lg`}>
              <Calendar className="h-4 w-4 sm:h-5 sm:w-5" />
              <span className="text-sm sm:text-base font-medium truncate max-w-[200px]">{today}</span>
            </div>
